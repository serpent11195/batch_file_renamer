...
def detect_conflicts(plan):
    ...
    for task in plan:
        ...
        if not is_valid_file_name(new_name):
            errors.append(f"Tên mới không hợp lệ: {new_name}")
            continue
        ...
        if new_key in seen_new_paths:
            ...
            errors.append(
                f"Hai file cùng đổi thành một tên: '{first_old}' và '{old_path}' -> '{new_path}'"
            )
        else:
        ...
        if new_path.exists() and new_key not in old_paths:
            errors.append(f"File đích đã tồn tại: {new_path}")
